...
* **Custom Data Structures:** Utilizes `struct` and `typedef` to bundle complex entity data (health, stats, XP) and `enum` to enforce strict elemental typing (Fire, Fire, Grass, etc.).
...
* **Modular Codebase:** Strictly separates declarations from definitions across multiple `.h` and `.c` files. Properly manages the `extern` keyword to prevent linker conflicts and maintain scope.
...
* `game.c` - World map, entity spawning, and global variables
* `menu.c` - Interface and player navigation
* `testing.c` - Sandbox and test environments
* `system.c` - Core system functions and data
* `battling.c` - Combat math, turn-based logic, and random numbers
...
git clone [https://github.com/Sparkening/ASCIImon.git](https://github.com/Sparkening/ASCIImon.git)

# 2. Navigate to the project directory
cd ASCIImon

# 3. Compile the source code
gcc main.c menu.c game.c testing.c system.c battling.c -o asciimon
